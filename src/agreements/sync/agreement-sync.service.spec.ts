...
const { relayToTrustless } = jest.requireMock(
  "../../internal-trustless/trustless-relay.helper",
);
...
describe("AgreementSyncService", () => {
  ...
  describe("syncStatusTransition", () => {
    it("should reject invalid transitions via validation service", async () => {
      ...
      const result = await service.syncStatusTransition(
        "test-id", "pending", "completed",
      );
      expect(result.synced).toBe(false);
    ...
    it("should validate and log valid transitions", async () => {
      ...
      expect(result.actions).toContain(
        'Transition "pending" → "funded" validated',
      );
    });
  });
